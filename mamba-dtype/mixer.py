...
from mconfig import MambaConfig
...
class MambaMixer(nn.Module):
    ...
    def __init__(self, config: MambaConfig, layer_idx: int = -1):
        ...
        self.in_proj = nn.Linear(self.hidden_size, self.intermediate_size * 2, bias=config.use_bias, dtype=MambaConfig.dtype)
        ...
        self.x_proj = nn.Linear(self.intermediate_size, self.time_step_rank + self.ssm_state_size * 2, bias=False, dtype=MambaConfig.dtype)
        ...
        self.dt_proj = nn.Linear(self.time_step_rank, self.intermediate_size, bias=True, dtype=MambaConfig.dtype)
        ...
        A = torch.arange(1, self.ssm_state_size + 1, dtype=MambaConfig.dtype)[None, :]
        A = A.expand(self.intermediate_size, -1).contiguous()

        self.A_log = nn.Parameter(torch.log(A))
        self.D = nn.Parameter(torch.ones(self.intermediate_size, dtype=MambaConfig.dtype))
        self.out_proj = nn.Linear(self.intermediate_size, self.hidden_size, bias=config.use_bias, dtype=MambaConfig.dtype)
...
class MambaModel(MambaPreTrainedModel):
    def __init__(self, config):
        ...
        # if not dist.is_initialized() or not dist.is_nccl_available():
        #     print('No distributed setup detected.')
